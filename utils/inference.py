...
from utils.data_io import MovieFeatureStore, clean_value, load_movie_feature_store, load_ratings, user_token
...
def load_inference_components(
    ...
    user_tokens_path = Path(model_name_or_path) / "user_tokens.json"
    if user_tokens_path.exists():
        user_tokens = [record["user_token"] for record in json.loads(user_tokens_path.read_text(encoding="utf-8"))]
        tokenizer.add_tokens(user_tokens, special_tokens=False)
...
def restrict_scores_to_candidates(
    ...
    for row_candidates in candidate_movie_ids:
        ids = components.movie_ids if row_candidates is None else [clean_value(movie_id) for movie_id in row_candidates]
...
def rank_movie_recommendations_batch(
    ...
    with torch.no_grad():
        ...
        movie_scores = outputs.logits[row_indices, last_indices, :].index_select(dim=-1, index=movie_token_ids)
    ...
    top_count = min(max(1, ranking_config.top_k), movie_scores.shape[1])
    ...
    for row in range(len(prompts)):
        ...
        results.append({"predicted_movie_ids": predicted_movie_ids, "predicted_movie_tokens": predicted_movie_tokens, "scores": scores})
    return results
